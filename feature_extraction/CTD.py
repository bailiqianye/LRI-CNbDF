...
if __name__ == "__main__":
    ...
    protein = readFASTAs(path+'/dataset/mitab/receptor.fasta')
    feature = np.zeros((1, 147))
    for i in protein:
        vector = list(CalculateCTD(i).values())
        feature = np.vstack([feature, vector])
    print(feature[1:])
    curve_1 = pd.DataFrame(feature[1:])
    curve_1.to_csv(path+'/dataset/mitab/receptor_CTD_feature.csv', header=None, index=None)
